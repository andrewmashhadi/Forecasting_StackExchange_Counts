...
## visualize the annual cycle with a few example years
...
## visualize the 40 month cycle with a few example years
...
# overlay fitted to counts training data
ts.plot(xt_trn, 
        type = "l", 
...
arma.res.spec <- mvspec(resid(arma.mod), 
                        log="no",
                        main="Series: ARMA(3, 8) Residuals | Raw Periodogram", 
                        lwd=2)
...
# output training RMSE for counts
cat("Training RMSE:", sqrt(mean((xt_trn - trnpreds)^2)), "\n\n")
